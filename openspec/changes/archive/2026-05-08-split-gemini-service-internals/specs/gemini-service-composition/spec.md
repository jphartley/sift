...
The system SHALL provide a Gemini prompt builder that constructs the practice recommendation prompt from the current transcript, practice library, and session history without requiring network-capable Gemini service initialization.
...
- **THEN** the resulting prompt SHALL include the complete practice library

#### Scenario: Prompt builder receives session history
- **WHEN** the prompt builder receives prior session history
